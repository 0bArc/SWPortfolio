...
import {
  ...
  grantKeyFor,
  stackSummary,
} from "@/features/accounts/services/badges/catalog";

export type { BadgeDef } from "@/features/accounts/services/badges/catalog";
export { BADGES, BADGE_BY_SLUG, BADGE_DEFS } from "@/features/accounts/services/badges/catalog";
...
export async function syncBadgesForAccount(accountId: number, username: string): Promise<void> {
  const { ensureBadge, countCommentsForAccount } = await import("@/database/accounts");
...
export async function syncBadgesForUsername(username: string): Promise<AccountBadge[]> {
  const { getAccountByUsername, listBadgesForAccount } = await import("@/database/accounts");
