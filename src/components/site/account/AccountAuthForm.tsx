...
import { AlertCircle, Lock, User, AtSign, ImageIcon } from "lucide-react";
...
export default function AccountAuthForm({ mode, turnstileSiteKey }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as {
        error?: string;
        account?: { username: string; displayName: string; icon: string | null };
      };
    ...
    <form onSubmit={handleSubmit} className="glass rounded-2xl border border-white/[0.1] overflow-hidden shadow-2xl shadow-black/40">
      <div className="px-7 pt-7 pb-5 border-b border-white/[0.06] bg-white/[0.01]">
        ...
        <p className="text-[12px] text-gray-500 mt-1">
          {isSignup
            ? "Comment on posts and get your own profile page."
            : "Welcome back."}
        </p>
      </div>
      ...
      <div className="p-7 space-y-4">
        ...
        {isSignup && (
          <div>
            <label htmlFor="acct-icon" className="text-[11px] font-medium text-gray-500 uppercase tracking-wider">
              Avatar URL <span className="normal-case text-gray-600">(optional)</span>
            </label>
            <div className="relative mt-1.5">
              <ImageIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-600 pointer-events-none" />
              <input
                id="acct-icon"
                type="url"
                className={`${inputClass} pl-9`}
        ...
        <button
          ...
          disabled={loading || !username || !password || (isSignup && !displayName)}
        >
